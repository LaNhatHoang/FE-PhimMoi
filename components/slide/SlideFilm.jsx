...
const SlideFilm = (props) => {
    const { countSlide, url, title, data } = props
    ...
    return (
        <div className="Slide mb-6">
            <header className="px-2.5 py-3.5 flex flex-col md:flex-row lg:flex-col xl:flex-row md:items-center md:justify-between lg:items-stretch xl:items-center">
                <h2 className="pl-2.5 border-l-[3px] border-blue-500 text-white text-xl font-semibold">{title}</h2>
                <div className="flex flex-wrap justify-end mt-2 md:m-0 lg:mt-2 xl:m-0 text-p text-lg">
                    <div className='text-[10px] uppercase text-white me-3 lg:me-0'>
                        <a href={url ? url : ""} className='bg-blue-500 px-2 py-1 rounded-sm'>Xem thêm</a>
                    </div>
            ...
            <div ref={wrapRef} className="Wrap overflow-hidden w-full border-b c-border">
                <div onMouseDown={handleMouseDown}
                    ...
                    ref={slideRef} className={
                        countSlide == 10 ? "Content h-full w-333 nm:w-250 lg:w-333 xl:w-250 3xl:w-200 block overflow-hidden" :
                            countSlide == 12 ? "Content h-full w-400 nm:w-300 lg:w-400 xl:w-300 3xl:w-240 block overflow-hidden" :
                                "Content h-full w-500 nm:w-375 lg:w-500 xl:w-375 3xl:w-300 block overflow-hidden"
                    }
                >
                    {
                        data.map((film, index) => (
                            <div key={index} className={
                                ...
                                <article className='p-[5px] md:p-2.5 w-full'>
                                    ...
                                    <div className="Data mt-3.5 mb-2.5">
                                        <h3 className='line-clamp-1 text-white text-sm hover:text-blue-500 font-medium'>
                                            {link ?
                                                <a href={film.type == 'movies' ? `/phim-le/${film.url}` : `/phim-bo/${film.url}`}>{film.name}</a>
                                                : <span>{film.name}</span>
                                            }
                                        </h3>
                                        <h3 className='line-clamp-1 text-p text-xs font-medium mt-1'>
                                            <span>{film.nameEng}</span>
                                        </h3>
                                    </div>
                                </article>
                            </div>
